...
class D_Net(nn.Module):
    def __init__(self):
        ...
        self.d_net = nn.Sequential(
            ...
            nn.LeakyReLU(0.2, True),  # 14,14

            nn.Conv2d(128, 256, 4, 2, 1, bias=False),
            nn.BatchNorm2d(256),
            nn.LeakyReLU(0.2, True),  # 7,7

            nn.Conv2d(256, 128, 4, 2, 1, bias=False),
            nn.BatchNorm2d(128),
            nn.LeakyReLU(0.2, True),  # 3,3

            nn.Conv2d(128, 10, 3, 1, 0, bias=False),
            nn.Sigmoid()
        )
...
class G_Net(nn.Module):
    def __init__(self):
        ...
        self.g_net = nn.Sequential(
            ...
            nn.ConvTranspose2d(128, 256, 4, 2, 1, 1, bias=False),
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True),
            ...
            nn.ConvTranspose2d(128, 1, 4, 2, 1, 0, bias=False),
            nn.Tanh()
        )
...
if __name__ == '__main__':
    ...
    for epoch in range(EPOCH):
        for i, (x, y) in enumerate(train_data):
            # print(y)
            labens_onehot = torch.zeros((BATCH_SIZE, 10))  # 造零矩阵
            # print(labens_onehot.size())
            labens_onehot[torch.arange(BATCH_SIZE), y] = 1  # 将标签对应的哪一个数的索引变成1，转onehot
            # print(labens_onehot)

            real_label = labens_onehot.to(device)
            fake_label = torch.zeros(BATCH_SIZE, 10).to(device)

            # 训练判别器
            real_out = d_net(x.to(device))

            real_out = real_out.reshape(real_out.size(0), -1)
            # print('1', real_out.size())
            # print('2', real_label.size())
            ...
            # 训练判别器
            c = torch.randn(BATCH_SIZE, 118)
            c = torch.cat((c, labens_onehot), dim=1).to(device)
            # print('c',c.size())
            c = c.reshape(z.size(0), -1, 1, 1)
            fake_imgs = g_net(c)
            ...
            if i % 10 == 0:
                ...
                save_image(x, 'pic/cgan/real_face_cgan_{}.jpg'.format(i), nrow=10, normalize=True, scale_each=True)
                save_image(fake_imgs, 'pic/cgan/fake_face_cgan_{}.jpg'.format(i), nrow=10, normalize=True,
                           scale_each=True)

        s.add_histogram('face_d_w_cgan', d_net.d_net[0].weight.data, global_step=epoch)
        s.add_histogram('face_g_w_cgan', g_net.g_net[0].weight.data, global_step=epoch)
        s.add_scalars('face_loss_cgan', {'face_d_loss_cgan': d_loss, 'face_g_loss_cgan': g_loss}, global_step=epoch)
